...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      INTEGER m
      PARAMETER(m=100)
      REAL lo,th,ls,hs,lr,hr,wa,va,an,EX,PE,PR,CR,CE,MA,RED,WA2 
      REAL exptime,rd

      REAL ga(m),re(m),hi(m),fw(m),fi(m),ps(m),is(m),os(m)
      CHARACTER logfile*30,image(m)*5,optfile(m)*9,alsoptfile(m)*13
      INTEGER i,n
...
C
      ...
      READ(1,*)            
      DO i = 1, 100
         READ(1,*,ERR=99) image(i),exptime,GA(i),RD,FW(i),HI(i)
         ...
         n = n + 1
      ENDDO
c10    FORMAT(A,F10.3,3F8.2,F10.0)
99    CLOSE(1)
...
60      FORMAT('FI = ',F8.2       /
     ...
     &         'MA = ',F8.2       /)
      ...
      subroutine trim_spaces(name,ipos) 
      IMPLICIT NONE
