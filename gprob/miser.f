      SUBROUTINE miser(func,region,ndim,npts,dith,ave,var,MTerr,MDerr
     &,Merr,MTo,MDo,Mo)
      implicit none
      INTEGER ndim,npts,MNPT,MNBS,MAXD,NSTACK,FLAG
      double precision ave,dith,var,region(2*ndim),func,TINY,BIG,PFAC
...
CU    USES func,ranpt
      ...
      double precision avel,fracl,fval,rgl,rgm,rgr,s,sigl,
     &siglb,sigr,sigrb,sum,sumb,
     ...
     *fracl)
      double precision MTerr,MDerr,Merr,MTo,MDo,Mo
      integer numout
      SAVE iran
      DATA iran /0/
      jstack=0
...
1     continue
      if (nptt.lt.MNBS) then
        ...
        do 11 n=1,np
          ...
          fval=func(pt,MTerr,MDerr,Merr,MTo,MDo,Mo,FLAG)
          if(FLAG.eq.1) then
             numout=numout+1
          endif
          summ=summ+fval
...
11      continue
        ave=summ/numout
        var=max(TINY,(summ2-summ**2/numout)/numout**2)
      else
